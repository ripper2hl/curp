...
describe('curp', () => {
  ...
  it('Deberia obtener el CURP correcto de Enrique Alfonso Ferrer Mac Gregor Gil', () => {
    const persona = curp.getPersona();
    persona.nombre = 'Enrique Alfonso';
    persona.apellidoPaterno = 'Ferrer';
    persona.apellidoMaterno = 'Mac Gregor Gil';
    persona.genero = curp.GENERO.MASCULINO;
    persona.fechaNacimiento = '01-08-1957';
    persona.estado = curp.ESTADO.CDMX;
    expect(curp.generar(persona)).toBe('FEGE570801HDFRLN08');
  });
  ...
  it('Deberia obtener el CURP correcto cuando el apellido es unicamente Mac Gregor', () => {
    const persona = curp.getPersona();
    persona.nombre = 'Luis';
    persona.apellidoPaterno = 'Mac Gregor';
    persona.apellidoMaterno = '';
    persona.genero = curp.GENERO.MASCULINO;
    persona.fechaNacimiento = '09-09-1975';
    persona.estado = curp.ESTADO.CDMX;
    // The previous implementation would return XPL750909HDFXRS05 with length 17, now MAXL...
    expect(curp.generar(persona).length).toBe(18);
    expect(curp.generar(persona)).toBe('MAXL750909HDFCXS02');
  });

  it('Deberia obtener el CURP correcto cuando el apellido es unicamente DE LA', () => {
    const persona = curp.getPersona();
    persona.nombre = 'Luis';
    persona.apellidoPaterno = 'DE LA';
    persona.apellidoMaterno = 'PEREZ';
    persona.genero = curp.GENERO.MASCULINO;
    persona.fechaNacimiento = '09-09-1975';
    persona.estado = curp.ESTADO.CDMX;
    expect(curp.generar(persona).length).toBe(18);
    expect(curp.generar(persona)).toBe('LAPL750909HDFXRS02');
  });

  it('Deberia obtener el CURP correcto cuando el apellido tiene prefijos en cascada (ej. DE LA MAC GREGOR)', () => {
    const persona = curp.getPersona();
    persona.nombre = 'Luis';
    persona.apellidoPaterno = 'DE LA MAC GREGOR';
    persona.apellidoMaterno = 'PEREZ';
    persona.genero = curp.GENERO.MASCULINO;
    persona.fechaNacimiento = '09-09-1975';
    persona.estado = curp.ESTADO.CDMX;
    // Si la función ajustaCompuesto falla, podría regresar "DE LA MAC GREGOR" (CURP: DEPL...)
    // Lo correcto es que elimine "DE LA" y al intentar eliminar "MAC GREGOR" (que lo dejaría vacío),
    // se detenga y devuelva "MAC GREGOR".
    // "MAC GREGOR": M (primera letra), A (vocal), P (Perez), L (Luis) -> MAPL...
    // Consonantes: C (Mac Gregor), R (Perez), S (Luis) -> CRS
    // Let's verify what the generated CURP should theoretically be if it's "MAC GREGOR":
    // "MAC GREGOR" -> M, A. "PEREZ" -> P. "LUIS" -> L. So MAPL.
    // Consonantes internas: "MAC GREGOR" -> C. "PEREZ" -> R. "LUIS" -> S. -> CRS.
    // Length must be 18.
    expect(curp.generar(persona).substring(0, 4)).toBe('MAPL');
  });
